...
function EditMemory({ userData, handleLogout, navigateTo, memoryId }) {
    ...
    useEffect(() => {
        const fetchMemoryData = async () => {
            try {
                ...
                // Fetch memory details with edit permission check
                const memoryRes = await axios.get(`http://localhost:5000/api/memories/${memoryId}`, {
                    headers: { 'Authorization': `Bearer ${token}` }
                });

                // Check if we have edit permissions from the response headers
                const canEdit = memoryRes.headers['x-can-edit'] === 'true';

                if (!canEdit) {
                    showMessage("You don't have permission to edit this memory");
                    navigateTo('MemoryDetail', memoryId);
                    return;
                }
        ...
        if (!memoryId) {
            console.error('No memoryId provided to EditMemory');
            navigateTo('Home'); // or handle this case appropriately
            return;
        }
    ...
    const saveChanges = async () => {
        ...
        try {
            ...
            if (error.response?.status !== 403) { // Don't show duplicate message
